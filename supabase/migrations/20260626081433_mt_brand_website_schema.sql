...
We collect information you provide directly to us when making a purchase.

## How We Use Your Information

We use the information to process transactions and provide customer support.

## Data Security

We implement measures to protect your personal information.
...
By using MT Brand website, you agree to these Terms of Service.
...
We offer a 30-day return policy on most items in original condition.

## Limitation of Liability

MT Brand shall not be liable for any indirect or consequential damages.',
...
我們對大多數原始狀態的商品提供30天退貨政策。

## 責任限制

MT品牌對因使用我們的產品或服務引起的損害不承擔責任。'),
...
Free shipping on all orders over HK$500. Standard fee of HK$30 applies below HK$500.
...
所有訂單滿HK$500免運費。HK$500以下收取HK$30標準運費。
...
INSERT INTO mt_products (name_en, name_tc, slug, description_en, description_tc, price, compare_price, images, collection_id, tags, inventory_qty, sku, is_active, is_featured) VALUES
('Premium Leather Wallet', '頂級皮革錢包', 'premium-leather-wallet', 'Handcrafted from full-grain leather, this slim wallet features 6 card slots and a cash compartment. Perfect for the modern professional.', '採用全粒面皮革手工製作，這款纖薄錢包配有6個卡槽和現金夾層，非常適合現代專業人士。', 680, 880, ARRAY['https://images.pexels.com/photos/1152077/pexels-photo-1152077.jpeg?auto=compress&cs=tinysrgb&w=800'], (SELECT id FROM mt_collections WHERE slug = 'premium-series'), ARRAY['leather','wallet','accessories'], 50, 'MT-WALLET-001', true, true),

('Minimalist Watch', '極簡主義手錶', 'minimalist-watch', 'A clean, understated timepiece with sapphire crystal glass and genuine leather strap. Water-resistant to 50m.', '一款簡潔低調的計時器，配有藍寶石水晶玻璃和真皮錶帶，防水深度達50米。', 1280, 1580, ARRAY['https://images.pexels.com/photos/190819/pexels-photo-190819.jpeg?auto=compress&cs=tinysrgb&w=800'], (SELECT id FROM mt_collections WHERE slug = 'premium-series'), ARRAY['watch','accessories','premium'], 25, 'MT-WATCH-001', true, true),

('Canvas Tote Bag', '帆布手提袋', 'canvas-tote-bag', 'A versatile everyday tote made from organic cotton canvas. Features an interior zip pocket and reinforced handles.', '一款多功能日常托特包，採用有機棉帆布製成，配有內置拉鍊口袋和加固把手。', 320, null, ARRAY['https://images.pexels.com/photos/1152077/pexels-photo-1152077.jpeg?auto=compress&cs=tinysrgb&w=800'], (SELECT id FROM mt_collections WHERE slug = 'lifestyle'), ARRAY['bag','canvas','everyday'], 100, 'MT-BAG-001', true, false),

('Ceramic Coffee Mug', '陶瓷咖啡杯', 'ceramic-coffee-mug', 'A beautifully crafted ceramic mug with a matte finish. Holds 350ml and is microwave and dishwasher safe.', '一款精美的啞光陶瓷杯，容量350ml，可用於微波爐和洗碗機。', 180, 220, ARRAY['https://images.pexels.com/photos/312418/pexels-photo-312418.jpeg?auto=compress&cs=tinysrgb&w=800'], (SELECT id FROM mt_collections WHERE slug = 'lifestyle'), ARRAY['mug','ceramic','kitchen'], 200, 'MT-MUG-001', true, false),

('Wool Blend Scarf', '羊毛混紡圍巾', 'wool-blend-scarf', 'A luxuriously soft scarf made from a premium wool and cashmere blend. Available in multiple colors.', '一款採用優質羊毛和開司米混紡製成的奢華柔軟圍巾，有多種顏色可選。', 450, 580, ARRAY['https://images.pexels.com/photos/1020370/pexels-photo-1020370.jpeg?auto=compress&cs=tinysrgb&w=800'], (SELECT id FROM mt_collections WHERE slug = 'new-arrivals'), ARRAY['scarf','wool','fashion'], 75, 'MT-SCARF-001', true, true),

('Stainless Steel Bottle', '不鏽鋼水瓶', 'stainless-steel-bottle', 'Double-wall vacuum insulated bottle keeps drinks cold for 24 hours or hot for 12 hours. BPA-free and eco-friendly.', '雙層真空隔熱水瓶，可保持飲料冷藏24小時或保溫12小時，不含BPA，環保。', 280, 350, ARRAY['https://images.pexels.com/photos/1000084/pexels-photo-1000084.jpeg?auto=compress&cs=tinysrgb&w=800'], (SELECT id FROM mt_collections WHERE slug = 'best-sellers'), ARRAY['bottle','hydration','eco'], 150, 'MT-BOTTLE-001', true, false),

('Leather Card Holder', '皮革名片夾', 'leather-card-holder', 'Ultra-slim card holder crafted from top-grain leather. Holds up to 8 cards with a smooth sliding mechanism.', '採用頂粒面皮革製成的超薄名片夾，滑動機制流暢，可容納多達8張卡片。', 380, null, ARRAY['https://images.pexels.com/photos/1152077/pexels-photo-1152077.jpeg?auto=compress&cs=tinysrgb&w=800'], (SELECT id FROM mt_collections WHERE slug = 'best-sellers'), ARRAY['leather','card holder','accessories'], 80, 'MT-CARD-001', true, false),

('Bamboo Desk Organizer', '竹製桌面收納盒', 'bamboo-desk-organizer', 'Keep your workspace tidy with this sustainable bamboo desk organizer. Features 5 compartments for pens, clips, and more.', '用這款可持續竹製桌面收納盒保持您的工作空間整潔，配有5個隔間，可放置筆、夾子等物品。', 240, 300, ARRAY['https://images.pexels.com/photos/667838/pexels-photo-667838.jpeg?auto=compress&cs=tinysrgb&w=800'], (SELECT id FROM mt_collections WHERE slug = 'new-arrivals'), ARRAY['bamboo','organizer','office'], 60, 'MT-DESK-001', true, false)
